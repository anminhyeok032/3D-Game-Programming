...
{
	//float fCubeWidth = fWidth / 3.0f;
	float fHalfWidth = fWidth * 0.6f;
	float fHalfHeight = fHeight * 0.2f;
	float fHalfDepth = fDepth * 0.6;
	int i = 0;
	...
	//-- Turret
	float fTurretWidth = fWidth * 0.4f;
	float fTurretHeight = fHeight * 0.15f;
	float fTurretDepth = fDepth * 0.4f;
	fHalfHeight *= 2.0f;

	CPolygon* pTurretFace = new CPolygon(4);
	pTurretFace->SetVertex(0, CVertex(-fTurretWidth, +fTurretHeight+fHalfHeight, +fTurretDepth));
	pTurretFace->SetVertex(1, CVertex(-fTurretWidth, +fTurretHeight+fHalfHeight, -fTurretDepth));
	pTurretFace->SetVertex(2, CVertex(-fTurretWidth, -fTurretHeight+fHalfHeight, -fTurretDepth));
	pTurretFace->SetVertex(3, CVertex(-fTurretWidth, -fTurretHeight+fHalfHeight, +fTurretDepth));
	SetPolygon(++i, pTurretFace);			  						 
																	 
																	
	pTurretFace = new CPolygon(4);									 
	pTurretFace->SetVertex(0, CVertex(-fTurretWidth, +fTurretHeight+fHalfHeight, +fTurretDepth));
	pTurretFace->SetVertex(1, CVertex(+fTurretWidth, +fTurretHeight+fHalfHeight, +fTurretDepth));
	pTurretFace->SetVertex(2, CVertex(+fTurretWidth, +fTurretHeight+fHalfHeight, -fTurretDepth));
	pTurretFace->SetVertex(3, CVertex(-fTurretWidth, +fTurretHeight+fHalfHeight, -fTurretDepth));
	...
	//-- Gun
	float fGunWidth = fWidth * 0.1f;
	float fGunHeight = fHeight * 0.1f;
	float fGunDepth = fDepth * 1.0f;

	CPolygon* pGunFace = new CPolygon(4);
	pGunFace->SetVertex(0, CVertex(-fGunWidth, +fGunHeight+fHalfHeight, +fGunDepth+7.0f));
	pGunFace->SetVertex(1, CVertex(-fGunWidth, +fGunHeight+fHalfHeight, -fGunDepth+7.0f));
	pGunFace->SetVertex(2, CVertex(-fGunWidth, -fGunHeight+fHalfHeight, -fGunDepth+7.0f));
	pGunFace->SetVertex(3, CVertex(-fGunWidth, -fGunHeight+fHalfHeight, +fGunDepth+7.0f));
	SetPolygon(++i, pGunFace);


	pGunFace = new CPolygon(4);
	pGunFace->SetVertex(0, CVertex(-fGunWidth, +fGunHeight+fHalfHeight, +fGunDepth+7.0f));
	pGunFace->SetVertex(1, CVertex(+fGunWidth, +fGunHeight+fHalfHeight, +fGunDepth+7.0f));
	pGunFace->SetVertex(2, CVertex(+fGunWidth, +fGunHeight+fHalfHeight, -fGunDepth+7.0f));
	pGunFace->SetVertex(3, CVertex(-fGunWidth, +fGunHeight+fHalfHeight, -fGunDepth+7.0f));
...
CFloorMesh::CFloorMesh(float fWidth, float fDepth, int nSubRects) : CMesh(nSubRects * nSubRects)
{
	float fHalfWidth = fWidth * 0.5f;
	float fHalfDepth = fDepth * 0.5f;
	float fCellWidth = fWidth * (1.0f / nSubRects) * 2;
